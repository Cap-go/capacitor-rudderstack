...
This file provides guidance to AI agents and contributors working on `@capgo/capacitor-rudderstack`.

## Public Release Requirements (Mandatory)

When shipping this plugin publicly, perform all of the following:
...
2. Set the GitHub repository description and ensure it starts with:
   - `Capacitor plugin for RudderStack ...`
3. Set the GitHub repository homepage to:
   - `https://capgo.app/docs/plugins/rudderstack/`
4. Open a pull request on `https://github.com/Cap-go/website` (or monorepo folder `landing/`) and update:
   ...
   - `src/content/docs/docs/plugins/rudderstack/ios.mdx` and `android.mdx` when platform setup differs
   - `astro.config.mjs` (pagefind bucket + docs sidebar entry)
   - `src/content/plugins-tutorials/en/capacitor-rudderstack.md` (SEO tutorial page)
   - `public/icons/plugins/rudderstack.svg` when the docs hero references a plugin icon
5. Keep the README Capgo CTA header block and replace:
   - the CTA tracking slug with `rudderstack`

Reference commands:

```bash
# Create public repo directly
gh repo create Cap-go/capacitor-rudderstack --public --source=. --remote=origin --push

# Or switch existing private repo to public
gh repo edit Cap-go/capacitor-rudderstack --visibility public --accept-visibility-change-consequences

# Enforce description + homepage
gh repo edit Cap-go/capacitor-rudderstack \
  --description "Capacitor plugin for RudderStack analytics, identity, and event tracking." \
  --homepage "https://capgo.app/docs/plugins/rudderstack/"
```
...
- Keep the JavaScript API source-of-truth in `src/definitions.ts` and regenerate the README docs with `bun run docgen`.
- Preserve the Cordova-compatible method signatures exposed from `src/index.ts` unless the package major version changes.
- The first Capacitor release intentionally does not implement Cordova `config.factories`; do not advertise native destination factory support unless it is added on both platforms.
- Always support both CocoaPods and Swift Package Manager on iOS.
